...
export const requestReply = async (tid: string, message: string, attachList: Array<IThreadAttachForm>, quotepid: string, token: string, dispatch: Dispatch) => {
    const { data } = await FrontendRequestPromise(
        { url: POST_REPLY_THREAD(tid), data: { message, quotepid, attach: attachList, token }, method: "POST" },
        dispatch
    );
...
export const requestEditReply = async (pid: string, message: string, attachList: Array<IThreadAttachForm>, token: string, dispatch: Dispatch) => {
    const { data } = await FrontendRequestPromise({ url: OPTIONS_POST(pid), data: { message, attach: attachList, token }, method: "PUT" }, dispatch);
    return data as IGeneralResponse;
};
